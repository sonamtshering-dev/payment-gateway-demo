...
const S = {
  ...
  topbar: { height:62, background:'#030d1f', borderBottom:'1px solid rgba(255,255,255,0.06)', display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0 16px', position:'sticky' as const, top:0, zIndex:50, flexWrap:'wrap' as const, gap:8 } as React.CSSProperties,
  body:   { padding:'16px', maxWidth:1200, margin:'0 auto' } as React.CSSProperties,
  tabs:   { display:'flex', gap:4, background:'#0f1d35', border:'1px solid rgba(255,255,255,0.06)', borderRadius:12, padding:4, marginBottom:20, flexWrap:'wrap' as const } as React.CSSProperties,
  tab:    (active:boolean) => ({ background:active?'rgba(59,130,246,0.12)':'transparent', color:active?'#3b82f6':'#64748b', border:'none', borderRadius:8, padding:'7px 12px', fontSize:12, fontWeight:active?700:400, cursor:'pointer', fontFamily:'DM Sans,sans-serif', transition:'all 0.15s' }) as React.CSSProperties,
  card:   { background:'#0f1d35', border:'1px solid rgba(255,255,255,0.06)', borderRadius:14, padding:24 } as React.CSSProperties,
  tableWrap: { overflowX:'auto' as const, WebkitOverflowScrolling:'touch' as any, margin:'0 -24px', padding:'0 24px' } as React.CSSProperties,
  grid4:  { display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(160px,1fr))', gap:12, marginBottom:20 } as React.CSSProperties,
  statCard: { background:'#0f1d35', border:'1px solid rgba(255,255,255,0.06)', borderRadius:12, padding:'16px 18px' } as React.CSSProperties,
  ...
  btn:    (color:string) => ({ background:`rgba(${color},0.08)`, border:`1px solid rgba(${color},0.2)`, borderRadius:8, padding:'6px 14px', color:`rgb(${color})`, fontSize:12, fontWeight:600, cursor:'pointer', fontFamily:'DM Sans,sans-serif', whiteSpace:'nowrap' as const }) as React.CSSProperties,
  tHead:  { display:'grid', padding:'10px 16px', borderBottom:'1px solid rgba(255,255,255,0.06)', fontSize:11, fontWeight:700, color:'#4b5563', textTransform:'uppercase' as const, letterSpacing:'0.08em', minWidth:560 } as React.CSSProperties,
  tRow:   { padding:'13px 16px', borderBottom:'1px solid rgba(255,255,255,0.04)', fontSize:13, alignItems:'center', transition:'background 0.15s', minWidth:560 } as React.CSSProperties,
  badge:  (color:string) => ({ background:`rgba(${color},0.10)`, color:`rgb(${color})`, fontSize:10, fontWeight:700, padding:'3px 9px', borderRadius:100, letterSpacing:'0.05em', whiteSpace:'nowrap' as const }) as React.CSSProperties,
  overlay:{ position:'fixed' as const, inset:0, background:'rgba(0,0,0,0.75)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:100, padding:16 } as React.CSSProperties,
  modal:  { background:'#0f1d35', border:'1px solid rgba(255,255,255,0.08)', borderRadius:16, padding:24, width:520, maxWidth:'100%', maxHeight:'90vh', overflowY:'auto' as const } as React.CSSProperties,
};
...
export default function AdminPage() {
  ...
  const updateSubStatus = async (merchantId: string, status: string) => {
    const r = await fetch(`/api/v1/admin/subscriptions/${merchantId}/status`,{method:'PUT',headers,body:JSON.stringify({status})});
    const d = await r.json();
    if (!r.ok) { flash(d.error||'Failed',true); return; }
    flash(`Subscription ${status}`);
  };

  const saveSubChanges = async () => {
    if (!subModal || !subPlanId) return;
    setSubSaving(true);
    const r = await fetch(`/api/v1/admin/subscriptions/${subModal.id}/plan`,{method:'PUT',headers,body:JSON.stringify({plan_id:subPlanId,duration_days:subDays})});
    const d = await r.json();
    setSubSaving(false);
    if (!r.ok) { flash(d.error||'Failed',true); return; }
    setSubModal(null);
  ...
  const openSubModal = (m: any) => {
    setSubModal(m);
    setSubPlanId('');
    setSubDays(30);
  };

  const openNewPlan = () => { setPlanForm(EMPTY_PLAN); setPlanModal('new'); };
  ...
  const savePlan = async () => {
    ...
    const body = {
      name: planForm.name,
      price: Math.round(parseFloat(planForm.price||'0') * 100),
    ...
    <div style={S.page}>
      ...
      <div style={S.topbar}>
        <div style={{display:'flex',alignItems:'center',gap:10}}>
          <div style={{width:32,height:32,borderRadius:9,background:'linear-gradient(135deg,#3b82f6,#1d4ed8)',display:'flex',alignItems:'center',justifyContent:'center',color:'#020817',fontWeight:800,fontSize:14,flexShrink:0}}>N</div>
          <span style={{fontFamily:'Syne,sans-serif',fontSize:16,fontWeight:800}}>NovaPay</span>
          <span style={{background:'rgba(239,68,68,0.10)',color:'#ef4444',fontSize:10,fontWeight:700,padding:'3px 8px',borderRadius:6}}>ADMIN</span>
        </div>
        <div style={{display:'flex',alignItems:'center',gap:8}}>
          <div style={{background:'rgba(59,130,246,0.08)',border:'1px solid rgba(59,130,246,0.2)',color:'#3b82f6',fontSize:11,fontWeight:700,padding:'5px 10px',borderRadius:8}}>● Live</div>
          <button onClick={logout} style={{background:'none',border:'1px solid rgba(255,255,255,0.08)',borderRadius:8,padding:'6px 12px',color:'#64748b',cursor:'pointer',fontSize:12,display:'flex',alignItems:'center',gap:5}}>
          ...
          <>
              ...
              {[
                ...
                <div key={c.label} style={S.statCard}>
                  <div style={{display:'flex',justifyContent:'space-between',marginBottom:8}}>
                    <div style={{fontSize:10,color:'#64748b',fontWeight:700,textTransform:'uppercase' as const,letterSpacing:'0.08em'}}>{c.label}</div>
                    <span style={{fontSize:16}}>{c.icon}</span>
                  </div>
                  <div style={{fontFamily:'Syne,sans-serif',fontSize:22,fontWeight:800,color:c.color,letterSpacing:-1}}>{c.value}</div>
                </div>
              ))}
            </div>
            <div style={{...S.card,marginTop:0}}>
              ...
              <div style={{display:'flex',gap:10,flexWrap:'wrap' as const}}>
                {TABS.slice(1).map(t=>(
                  <button key={t} onClick={()=>setTab(t)} style={{background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.08)',borderRadius:10,padding:'10px 16px',color:'#8b9ab5',cursor:'pointer',fontSize:13,fontFamily:'DM Sans,sans-serif'}}>
                    {t} →
                  </button>
                ))}
              </div>
        ...
        {tab==='Merchants' && (
          <div style={S.card}>
            <div style={S.tableWrap}>
              ...
              {filtered(merchantList).length===0 && <div style={{padding:'48px',textAlign:'center' as const,color:'#4b5563',minWidth:560}}>No merchants found</div>}
        ...
        {tab==='KYC' && (
          <div style={S.card}>
            <div style={S.tableWrap}>
              ...
              {filtered(kycList).length===0 && <div style={{padding:'48px',textAlign:'center' as const,color:'#4b5563',minWidth:560}}>No KYC submissions yet</div>}
        ...
        {tab==='Fraud Alerts' && (
          <div style={S.card}>
            <div style={S.tableWrap}>
              ...
              {filtered(fraudList).length===0 && <div style={{padding:'48px',textAlign:'center' as const,color:'#4b5563',minWidth:560}}>No fraud alerts</div>}
        ...
        {tab==='Subscriptions' && (
          <div style={S.card}>
            <div style={{marginBottom:16}}>
              <div style={{fontFamily:'Syne,sans-serif',fontSize:18,fontWeight:800,marginBottom:4}}>Subscription Management</div>
              <div style={{color:'#64748b',fontSize:13}}>Activate, deactivate or change plans for merchants.</div>
            </div>
            <div style={S.tableWrap}>
              <div style={{...S.tHead,gridTemplateColumns:'1.5fr 1.5fr 0.8fr 1fr 1.5fr'}}>
              ...
              {filtered(merchantList).map((m:any)=>(
                ...
                >
                  ...
                  <div style={{display:'flex',gap:6,flexWrap:'wrap' as const}}>
                    <button onClick={()=>updateSubStatus(m.id,'active')} style={S.btn('0,229,176')}>Activate</button>
                    <button onClick={()=>updateSubStatus(m.id,'cancelled')} style={S.btn('239,68,68')}>Deactivate</button>
                    <button onClick={()=>openSubModal(m)} style={S.btn('139,92,246')}>Change Plan</button>
                    <button onClick={()=>setExtendModal(m)} style={S.btn('59,130,246')}>Extend</button>
                  </div>
                </div>
              ))}
            </div>
        ...
        {tab==='Plans' && (
          <div>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20,flexWrap:'wrap' as const,gap:12}}>
              <div>
                <div style={{fontFamily:'Syne,sans-serif',fontSize:18,fontWeight:800}}>Pricing Plans</div>
                <div style={{color:'#64748b',fontSize:13,marginTop:4}}>Manage the plans shown on your landing page.</div>
              </div>
              <button onClick={openNewPlan} style={{background:'linear-gradient(135deg,#3b82f6,#1d4ed8)',border:'none',borderRadius:10,padding:'10px 18px',color:'#020817',fontFamily:'Syne,sans-serif',fontSize:13,fontWeight:800,cursor:'pointer',display:'flex',alignItems:'center',gap:8}}>
            ...
            {planList.length===0 ? (
              <div style={{...S.card,textAlign:'center' as const,padding:48}}>
                ...
                <div style={{color:'#64748b',fontSize:14,marginBottom:24}}>Create your first pricing plan.</div>
              ...
              <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))',gap:16}}>
                {planList.map((plan:any)=>(
                  <div key={plan.id} style={{background:'#0f1d35',border:plan.is_featured?'1px solid rgba(59,130,246,0.3)':'1px solid rgba(255,255,255,0.06)',borderRadius:16,padding:20,position:'relative' as const}}>
                    {plan.badge && (
                      <div style={{position:'absolute' as const,top:14,right:14,background:'#3b82f6',color:'#020817',fontSize:9,fontWeight:800,padding:'4px 10px',borderRadius:5}}>
        ...
        <div style={S.overlay}>
          <div style={S.modal}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
              <h2 style={{fontFamily:'Syne,sans-serif',fontSize:17,fontWeight:800,margin:0}}>{planModal==='new'?'Create New Plan':'Edit Plan'}</h2>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14,marginBottom:14}}>
              <div><label style={S.label}>Plan Name *</label><input style={S.inp} value={planForm.name} onChange={e=>pf('name',e.target.value)} placeholder="e.g. Starter"/></div>
              <div><label style={S.label}>Price (₹)</label><input style={S.inp} type="number" min="0" value={planForm.price} onChange={e=>pf('price',e.target.value)} placeholder="999"/></div>
            </div>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14,marginBottom:14}}>
              <div>
              ...
              <div><label style={S.label}>Badge (optional)</label><input style={S.inp} value={planForm.badge} onChange={e=>pf('badge',e.target.value)} placeholder="Most Popular"/></div>
            </div>
            <div style={{marginBottom:14}}><label style={S.label}>CTA Button Label *</label><input style={S.inp} value={planForm.cta_label} onChange={e=>pf('cta_label',e.target.value)} placeholder="Get started"/></div>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:10,marginBottom:14}}>
              <div><label style={S.label}>QR Limit</label>
                <select style={S.inp} value={planForm.qr_limit} onChange={e=>pf('qr_limit',e.target.value)}>
                  <option value="0">∞ Unlimited</option><option value="50">50</option><option value="100">100</option><option value="250">250</option><option value="500">500</option><option value="1000">1,000</option>
                </select>
              </div>
              <div><label style={S.label}>Link Limit</label>
                <select style={S.inp} value={planForm.link_limit} onChange={e=>pf('link_limit',e.target.value)}>
                  <option value="0">∞ Unlimited</option><option value="1">1</option><option value="3">3</option><option value="5">5</option><option value="10">10</option><option value="25">25</option><option value="50">50</option>
                </select>
              </div>
              <div><label style={S.label}>API / day</label>
                <select style={S.inp} value={planForm.api_limit} onChange={e=>pf('api_limit',e.target.value)}>
                  <option value="0">∞ Unlimited</option><option value="100">100</option><option value="500">500</option><option value="1000">1,000</option><option value="5000">5,000</option><option value="10000">10,000</option>
                </select>
              </div>
            </div>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14,marginBottom:14}}>
              <div><label style={S.label}>Sort Order</label><input style={S.inp} type="number" min="0" value={planForm.sort_order} onChange={e=>pf('sort_order',e.target.value)}/></div>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14,marginBottom:14}}>
              <div><label style={S.label}>6-Month Discount %</label><input style={S.inp} type='number' min='0' max='100' value={planForm.discount_6month} onChange={e=>pf('discount_6month',e.target.value)}/></div>
              <div><label style={S.label}>1-Year Discount %</label><input style={S.inp} type='number' min='0' max='100' value={planForm.discount_1year} onChange={e=>pf('discount_1year',e.target.value)}/></div>
            </div>
            <div style={{marginBottom:20}}>
              <label style={S.label}>Features (one per line)</label>
              <textarea style={{...S.inp, minHeight:90, resize:'vertical' as const}} value={planForm.features} onChange={e=>pf('features',e.target.value)} placeholder={"100 QR codes\n5 payment links\nEmail support"}/>
            </div>
            <div style={{display:'flex',gap:10}}>
              <button onClick={()=>setPlanModal(null)} style={{flex:1,background:'none',border:'1px solid rgba(255,255,255,0.08)',borderRadius:10,padding:'11px 0',color:'#64748b',cursor:'pointer',fontSize:14}}>Cancel</button>
        ...
        <div style={S.overlay}>
          <div style={{...S.modal,width:440}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
              <h2 style={{fontFamily:'Syne,sans-serif',fontSize:18,fontWeight:800}}>Manage Subscription</h2>
              <button onClick={()=>setSubModal(null)} style={{background:'none',border:'none',color:'#64748b',cursor:'pointer'}}><X size={18}/></button>
            </div>
            <p style={{color:'#64748b',fontSize:13,marginBottom:20}}>{subModal.name} — {subModal.email}</p>

            <label style={S.label}>Change Plan</label>
            <select value={subPlanId} onChange={e=>setSubPlanId(e.target.value)} style={{...S.inp,marginBottom:16}}>
              <option value="">— Select a plan —</option>
              {plans.map((p:any)=>(
                <option key={p.id} value={p.id}>{p.name} — ₹{(p.price/100).toLocaleString('en-IN')}/{p.billing_cycle}</option>
              ))}
            </select>

            <label style={S.label}>Duration (days) — 0 = use plan default</label>
            <input type="number" min={0} value={subDays} onChange={e=>setSubDays(parseInt(e.target.value)||0)} style={{...S.inp,marginBottom:20}}/>

            <div style={{display:'flex',gap:10}}>
              <button onClick={()=>setSubModal(null)} style={{flex:1,background:'none',border:'1px solid rgba(255,255,255,0.08)',borderRadius:10,padding:'10px 0',color:'#64748b',cursor:'pointer',fontSize:14}}>Cancel</button>
              <button onClick={saveSubChanges} disabled={!subPlanId||subSaving} style={{flex:2,background:subPlanId?'linear-gradient(135deg,#8b5cf6,#6d28d9)':'#1e293b',border:'none',borderRadius:10,padding:'10px 0',color:subPlanId?'#fff':'#4b5563',fontFamily:'Syne,sans-serif',fontSize:14,fontWeight:800,cursor:subPlanId?'pointer':'not-allowed'}}>
                {subSaving ? 'Saving...' : 'Update Plan'}
              </button>
            </div>
          </div>
        </div>
