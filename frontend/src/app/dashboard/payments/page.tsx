      ...
      {result && (
        <div style={S.card}>
          ...
          <div style={S.linkBox}>
            ...
            <button style={S.copyBtn} onClick={() => copy(payLink)}>{copied ? '✓ Copied!' : 'Copy'}</button>
          </div>
          ...
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' as const }}>
            <button style={{ ...S.copyBtn, padding: '9px 16px', fontSize: 13 }} onClick={() => copy(result.upi_intent_link || '')}>Copy UPI Link</button>
            <button style={{ ...S.copyBtn, padding: '9px 16px', fontSize: 13 }} onClick={() => window.open(payLink, '_blank')}>Open Payment Page ↗</button>
          </div>
          ...
          <div style={S.history}>
            {history.map((p: any) => (
              <div key={p.id} style={S.histItem}>
                ...
                <button style={{ ...S.copyBtn, fontSize: 11 }} onClick={() => window.open(`/pay/${p.id}`, '_blank')}>View</button>
              </div>
            ))}
          </div>
        </div>
