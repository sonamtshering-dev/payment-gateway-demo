  ...
  return (
    <div style={{ padding: 32, color: '#eef2ff', fontFamily: 'DM Sans, sans-serif', maxWidth: 960, margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 32 }}>
        ...
        <button onClick={openAdd} style={{ background: 'linear-gradient(135deg,#00e5b0,#0ea5e9)', border: 'none', borderRadius: 10, padding: '11px 22px', color: '#07090f', fontFamily: 'Syne, sans-serif', fontSize: 14, fontWeight: 800, cursor: 'pointer' }}>
          + Connect Merchant
        </button>
      </div>
      ...
      {showForm && (
        <div style={{ position: 'fixed' as const, inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div style={{ background: '#111827', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 16, padding: 32, width: 440, maxWidth: '90vw' }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={handleSave} disabled={saving} style={{ flex: 2, background: saving ? 'rgba(255,255,255,0.05)' : 'linear-gradient(135deg,#00e5b0,#0ea5e9)', border: 'none', borderRadius: 10, padding: '11px 0', color: saving ? '#4b5563' : '#07090f', fontFamily: 'Syne, sans-serif', fontSize: 14, fontWeight: 800, cursor: saving ? 'not-allowed' : 'pointer' }}>
      ...
      {loading ? (
        ...
        <div style={{ background: '#111827', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 14, padding: '64px 24px', textAlign: 'center' as const }}>
          ...
          <button onClick={openAdd} style={{ background: 'linear-gradient(135deg,#00e5b0,#0ea5e9)', border: 'none', borderRadius: 10, padding: '11px 28px', color: '#07090f', fontFamily: 'Syne, sans-serif', fontSize: 14, fontWeight: 800, cursor: 'pointer' }}>Connect Merchant</button>
        </div>
      ) : (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 16 }}>
          {providers.map(p => (
            <div key={p.id} style={{ background: '#111827', border: `1px solid ${p.is_active ? 'rgba(0,229,176,0.12)' : 'rgba(255,255,255,0.06)'}`, borderRadius: 14, padding: 24, opacity: p.is_active ? 1 : 0.6, transition: 'all 0.2s' }}>
              {/* header */}
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
                ...
                <span style={{ background: p.is_active ? 'rgba(0,229,176,0.10)' : 'rgba(100,116,139,0.10)', color: p.is_active ? '#00e5b0' : '#64748b', fontSize: 10, fontWeight: 700, padding: '3px 9px', borderRadius: 100, letterSpacing: '0.06em' }}>
              ...
              {[['Merchant ID', p.merchant_mid || '—'], ['UPI ID', p.upi_id], ['Default', p.is_default ? 'Yes' : 'No']].map(([k, v]) => (
                <div key={k} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, marginBottom: 10 }}>
                  ...
                  <span style={{ color: '#eef2ff', fontWeight: 500, fontFamily: k === 'UPI ID' ? 'monospace' : 'inherit', fontSize: k === 'UPI ID' ? 12 : 13 }}>{v}</span>
                </div>
              ))}
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 20, borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: 16 }}>
                ...
                <button onClick={() => handleToggle(p)} style={{ flex: 1, background: p.is_active ? 'rgba(245,158,11,0.08)' : 'rgba(0,229,176,0.08)', border: `1px solid ${p.is_active ? 'rgba(245,158,11,0.20)' : 'rgba(0,229,176,0.20)'}`, borderRadius: 8, padding: '8px 0', color: p.is_active ? '#f59e0b' : '#00e5b0', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
