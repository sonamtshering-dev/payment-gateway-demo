...
const Logo = ({ size = 32 }: { size?: number }) => (
  <img src="/novapaylogo.jpeg" alt="NovaPay logo" style={{ width: size, height: size, objectFit: 'contain', borderRadius: 6 }} />
);
...
const FEATURES = [
  {
    ...
    desc: 'Money lands directly in your UPI account in under 2 seconds. No intermediary holds, no waiting — straight to your bank.',
    badge: null,
  },
  {
    ...
    desc: 'AES-256 encryption, KYC verification, IP whitelisting and AI-powered fraud detection built in.',
    badge: null,
  },
  {
  ...
  {
    icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#2563EB" strokeWidth="2" strokeLinecap="round"><path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z"/></svg>,
    title: 'Telegram Alerts',
    desc: 'Get instant payment notifications on Telegram. Connect your bot and never miss a transaction.',
    badge: 'NEW',
  },
  {
    icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#2563EB" strokeWidth="2" strokeLinecap="round"><circle cx="12" cy="12" r="10"/><path d="M9.09 9a3 3 0 015.83 1c0 2-3 3-3 3"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>,
    title: 'AI Assistant',
    desc: 'Ask your dashboard anything — transaction trends, success rates, revenue insights. Powered by AI.',
    badge: 'NEW',
  },
  {
    icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#2563EB" strokeWidth="2" strokeLinecap="round"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87"/><path d="M16 3.13a4 4 0 010 7.75"/></svg>,
    title: 'Team Management',
    desc: 'Add team members with role-based access. Cashiers, managers — everyone on one dashboard.',
    badge: 'NEW',
  },
  {
    icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#2563EB" strokeWidth="2" strokeLinecap="round"><circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/></svg>,
    title: 'Crypto Payments',
    desc: 'Accept USDT on TRC20, ERC20, and BNB networks alongside UPI. One gateway, all methods.',
    badge: 'NEW',
  },
  {
  ...
  {
    icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#2563EB" strokeWidth="2" strokeLinecap="round"><circle cx="12" cy="8" r="6"/><path d="M8 14l-4 8M16 14l4 8"/></svg>,
    title: 'White-label Branding',
    desc: 'Add your logo, business name and brand colors. Your customers see your brand on every checkout.',
    badge: 'NEW',
  },
];

const HOW_IT_WORKS = [
  { step: '01', title: 'Create Account', desc: 'Sign up free in 60 seconds. Complete KYC, add your UPI ID and customize your branding.' },
  { step: '02', title: 'Generate Payment Link', desc: 'Create a payment link or QR code from your dashboard. Share via WhatsApp, email, or SMS.' },
  { step: '03', title: 'Collect & Track', desc: 'Money hits your UPI directly. Get instant Telegram alerts and track everything in real-time.' },
];

const FAQ_ITEMS = [
  {
    q: 'Does NovaPay hold my payments?',
    a: 'No — never. When a customer pays via UPI, funds go directly from their bank to your UPI-linked bank account. NovaPay generates the payment link or QR code connected to your UPI ID; the money never passes through us.',
  },
  {
    q: 'Are there any transaction fees?',
    a: '0% — always. No per-transaction charges, no setup fee, no annual fee, no hidden costs. Optional subscription plans unlock higher limits and premium features, but the free plan is genuinely free.',
  },
  {
    q: 'How fast does money reach my account?',
    a: 'Instantly. Since payments route directly to your UPI ID, settlement happens in under 2 seconds — the same speed as any regular UPI transfer between individuals. No waiting period.',
  },
  {
    q: 'Who can use NovaPay?',
    a: 'Any individual, freelancer, small business, or enterprise in India with a valid UPI ID. Complete a quick KYC, add your UPI ID, and start collecting payments the same day — no paperwork, no waiting.',
  },
  {
    q: 'Can I integrate NovaPay into my website?',
    a: 'Yes. NovaPay provides a REST API, a Node.js/JavaScript SDK, a PHP SDK, and an official WooCommerce plugin. Integration typically takes under 30 minutes. Full developer documentation is available inside the dashboard.',
  },
  {
    q: 'What UPI IDs are supported?',
    a: 'Any NPCI-registered VPA works — @paytm, @ybl, @upi, @okaxis, @oksbi, @okhdfcbank, @ibl, and all others. You can also use a UPI ID linked to a business current account.',
  },
  {
    q: 'Is there a free plan?',
    a: 'Yes. The Starter plan is free forever — includes 100 QR codes, 5 payment links, and 500 API calls per day. No credit card required. Upgrade to Pro for unlimited usage, AI insights, team management, and white-label branding.',
  },
  {
    q: 'Is NovaPay secure?',
    a: 'Yes. AES-256 encryption on all data, KYC verification for every merchant, IP whitelisting, webhook signature validation, and AI-powered fraud detection. Customer payment data is processed entirely by UPI infrastructure and never stored on NovaPay servers.',
  },
];
      ...
      .then(d => {
        if (d.success && d.data) {
          setPlans(d.data.map((p: any) => ({
            ...
            link_limit: p.link_limit ?? -1,
            api_limit: p.api_limit ?? -1,
          })));
        }
      })
      .catch(() => {});
    ...
    <div style={{ background: '#fff', color: '#0F172A', fontFamily: "-apple-system,'Inter','Helvetica Neue',sans-serif", overflowX: 'hidden' }}>
      ...
      <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 200, height: 64, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 48px', background: scrolled ? 'rgba(255,255,255,0.97)' : '#fff', backdropFilter: 'blur(12px)', borderBottom: '1px solid #E2E8F0', transition: 'box-shadow .3s', boxShadow: scrolled ? '0 2px 16px rgba(0,0,0,.06)' : 'none' }}>
        ...
        <div className="desktop-nav" style={{ display: 'flex', gap: 32 }}>
          {[['Features','#features'],['How It Works','#how'],['Pricing','#pricing'],['FAQ','#faq'],['About','/about'],['Contact','/contact']].map(([l,h]) => (
          ...
          <div className="hero-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 440px', gap: 64, alignItems: 'center' }}>
            <div className="hero-anim">
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 7, fontSize: 12, fontWeight: 600, color: '#2563EB', background: '#DBEAFE', border: '1px solid #BFDBFE', borderRadius: 100, padding: '5px 14px', marginBottom: 28, letterSpacing: '.03em' }}>
                ...
                India's zero-fee UPI payment gateway
              </div>
              <h1 className="hero-title" style={{ fontSize: 56, fontWeight: 800, lineHeight: 1.06, letterSpacing: -2.5, color: '#0F172A', marginBottom: 20 }}>
                Accept UPI payments.<br/>Keep every rupee.
              </h1>
              <p style={{ fontSize: 18, color: '#64748B', lineHeight: 1.7, marginBottom: 40, maxWidth: 480 }}>
                Generate payment links and QR codes connected directly to your UPI ID. Money hits your bank account in under 2 seconds — no intermediary, zero transaction fees, instant settlement.
              </p>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 24, flexWrap: 'wrap' as const }}>
                {[['0%','Transaction fee'],['< 2s','Direct settlement'],['99.9%','Uptime']].map(([n,l]) => (
      ...
      <div style={{ borderBottom: '1px solid #E2E8F0', background: '#fff' }}>
        <div className="stats-grid" style={{ maxWidth: 1160, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(4,1fr)' }}>
          {[
            ...
            ['< 2s','Direct Settlement','Straight to your UPI account'],
      ...
      <div id="features" className="section-pad" style={{ maxWidth: 1160, margin: '0 auto', padding: '88px 48px' }}>
        <div style={{ textAlign: 'center' as const, marginBottom: 56 }}>
          ...
          <p style={{ fontSize: 16, color: '#64748B', maxWidth: 480, margin: '0 auto', lineHeight: 1.65 }}>Everything you need to accept UPI payments online, track revenue, and grow — no extra tools required.</p>
        </div>
        <div className="feat-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 16 }}>
          {FEATURES.map(f => (
            <div key={f.title} className="feat-card" style={{ position: 'relative' as const }}>
              {f.badge && (
                <div style={{ position: 'absolute' as const, top: 16, right: 16, fontSize: 9, fontWeight: 800, color: '#2563EB', background: '#DBEAFE', border: '1px solid #BFDBFE', borderRadius: 100, padding: '2px 8px', letterSpacing: '.06em' }}>{f.badge}</div>
              )}
      ...
      <div id="how" style={{ background: '#F8FAFC', borderTop: '1px solid #E2E8F0', borderBottom: '1px solid #E2E8F0', padding: '88px 48px' }}>
        <div className="section-pad" style={{ maxWidth: 1160, margin: '0 auto', padding: 0 }}>
          <div style={{ textAlign: 'center' as const, marginBottom: 56 }}>
            ...
            <h2 style={{ fontSize: 38, fontWeight: 800, letterSpacing: -1.5, color: '#0F172A', marginBottom: 12 }}>Start collecting payments in minutes</h2>
            <p style={{ fontSize: 16, color: '#64748B', maxWidth: 440, margin: '0 auto', lineHeight: 1.65 }}>No complex integration. No waiting. Accept UPI payments online the same day you sign up.</p>
          </div>
          ...
          {/* Direct settlement explainer */}
          <div style={{ marginTop: 48, background: 'linear-gradient(135deg, #EFF6FF, #F0FDF4)', border: '1.5px solid #BFDBFE', borderRadius: 20, padding: '32px 40px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 0, flexWrap: 'wrap' as const }}>
            <div style={{ textAlign: 'center' as const, flex: '1 1 180px', padding: '8px 20px' }}>
              <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 4 }}><svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="#64748B" strokeWidth="1.5" strokeLinecap="round"><circle cx="12" cy="8" r="4"/><path d="M4 20c0-4 3.6-7 8-7s8 3 8 7"/></svg></div>
              <div style={{ fontSize: 13, fontWeight: 700, color: '#0F172A', marginTop: 8 }}>Customer</div>
              <div style={{ fontSize: 11, color: '#64748B' }}>Pays via UPI app</div>
            </div>
            <div style={{ fontSize: 24, color: '#BFDBFE', padding: '0 8px' }}>→</div>
            <div style={{ textAlign: 'center' as const, flex: '1 1 180px', padding: '8px 20px', background: '#EFF6FF', borderRadius: 14, border: '1.5px solid #BFDBFE' }}>
              <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 4 }}><svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="#2563EB" strokeWidth="1.5" strokeLinecap="round"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z"/></svg></div>
              <div style={{ fontSize: 13, fontWeight: 700, color: '#2563EB', marginTop: 8 }}>UPI Network</div>
              <div style={{ fontSize: 11, color: '#64748B' }}>Instant routing</div>
            </div>
            <div style={{ fontSize: 24, color: '#BBF7D0', padding: '0 8px' }}>→</div>
            <div style={{ textAlign: 'center' as const, flex: '1 1 180px', padding: '8px 20px' }}>
              <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 4 }}><svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="#059669" strokeWidth="1.5" strokeLinecap="round"><rect x="3" y="10" width="18" height="11" rx="1"/><path d="M7 10V7a5 5 0 0110 0v3"/><line x1="12" y1="14" x2="12" y2="17"/></svg></div>
              <div style={{ fontSize: 13, fontWeight: 700, color: '#059669', marginTop: 8 }}>Your Bank Account</div>
              <div style={{ fontSize: 11, color: '#64748B' }}>In under 2 seconds</div>
            </div>
            <div style={{ width: '100%', textAlign: 'center' as const, marginTop: 20, fontSize: 13, color: '#475569', fontWeight: 500 }}>
              NovaPay never holds or touches your money — it goes <b style={{ color: '#059669' }}>directly to your UPI-linked account</b>.
            </div>
          </div>
        </div>
      </div>
      ...
      {/* FAQ */}
      <div id="faq" className="section-pad" style={{ maxWidth: 1160, margin: '0 auto', padding: '88px 48px' }}>
        <div style={{ textAlign: 'center' as const, marginBottom: 56 }}>
          <div style={{ display: 'inline-block', fontSize: 12, fontWeight: 700, color: '#2563EB', background: '#EFF6FF', border: '1px solid #BFDBFE', borderRadius: 100, padding: '4px 14px', letterSpacing: '.05em', marginBottom: 16 }}>FAQ</div>
          <h2 style={{ fontSize: 38, fontWeight: 800, letterSpacing: -1.5, color: '#0F172A', marginBottom: 12 }}>Common questions answered</h2>
          <p style={{ fontSize: 16, color: '#64748B', maxWidth: 480, margin: '0 auto', lineHeight: 1.65 }}>Everything you need to know about accepting UPI payments with NovaPay.</p>
        </div>
        <div className="faq-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 16 }}>
          {FAQ_ITEMS.map(faq => (
            <div key={faq.q} style={{ background: '#F8FAFC', border: '1.5px solid #E2E8F0', borderRadius: 16, padding: '24px 28px' }}>
              <div style={{ fontSize: 15, fontWeight: 700, color: '#0F172A', marginBottom: 10, lineHeight: 1.4 }}>{faq.q}</div>
              <div style={{ fontSize: 14, color: '#64748B', lineHeight: 1.75 }}>{faq.a}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* PRICING */}
      <div id="pricing" style={{ background: '#F8FAFC', borderTop: '1px solid #E2E8F0', padding: '88px 48px' }}>
        <div className="section-pad" style={{ maxWidth: 900, margin: '0 auto', padding: 0 }}>
          <div style={{ textAlign: 'center' as const, marginBottom: 56 }}>
            ...
            <p style={{ fontSize: 16, color: '#64748B', maxWidth: 400, margin: '0 auto', lineHeight: 1.65 }}>Start free, scale when you&apos;re ready. Cancel any time. 0% transaction fees on every plan.</p>
          </div>
          {plans.length > 0 ? (
            <div className="price-grid" style={{ display: 'grid', gridTemplateColumns: `repeat(${Math.min(plans.length,3)},1fr)`, gap: 16 }}>
              {plans.map(p => (
                <div key={p.name} style={{ background: p.featured ? 'linear-gradient(135deg,#1D4ED8,#2563EB)' : '#fff', border: p.featured ? 'none' : '1.5px solid #E2E8F0', borderRadius: 20, padding: '32px 28px', position: 'relative' as const, boxShadow: p.featured ? '0 12px 40px rgba(37,99,235,.3)' : '0 2px 8px rgba(0,0,0,.04)' }}>
                  ...
                  {/* Usage limits */}
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 6, marginBottom: 20, padding: '12px 0', borderTop: `1px solid ${p.featured ? 'rgba(255,255,255,.15)' : '#E2E8F0'}`, borderBottom: `1px solid ${p.featured ? 'rgba(255,255,255,.15)' : '#E2E8F0'}` }}>
                    {([['QR Codes', p.qr_limit, '/ period'], ['Pay Links', p.link_limit, 'active'], ['API Calls', p.api_limit, '/ day']] as [string, number, string][]).map(([l, v, unit]) => (
                      <div key={l} style={{ textAlign: 'center' as const }}>
                        <div style={{ fontSize: 9, fontWeight: 700, color: p.featured ? 'rgba(255,255,255,.5)' : '#94A3B8', textTransform: 'uppercase' as const, letterSpacing: '.07em', marginBottom: 3 }}>{l}</div>
                        <div style={{ fontSize: 15, fontWeight: 900, color: (v == null || v === -1) ? (p.featured ? '#86EFAC' : '#16A34A') : (p.featured ? '#fff' : '#0F172A') }}>{(v == null || v === -1) ? '∞' : v.toLocaleString('en-IN')}</div>
                        <div style={{ fontSize: 9, color: p.featured ? 'rgba(255,255,255,.4)' : '#94A3B8', marginTop: 1 }}>{unit}</div>
                      </div>
                    ))}
                  </div>
      ...
      <div style={{ background: 'linear-gradient(135deg,#1E40AF,#2563EB)', padding: '72px 48px', textAlign: 'center' as const }}>
        <h2 style={{ fontSize: 38, fontWeight: 800, letterSpacing: -1.5, color: '#fff', marginBottom: 14 }}>Ready to accept UPI payments?</h2>
        <p style={{ fontSize: 16, color: 'rgba(255,255,255,.75)', lineHeight: 1.7, marginBottom: 32, maxWidth: 460, margin: '0 auto 32px' }}>
          Join 500+ Indian merchants collecting UPI payments with NovaPay — zero transaction fees, direct settlement, instant setup.
        </p>
      ...
      <footer style={{ background: '#0F172A', padding: '48px', color: 'rgba(255,255,255,.5)' }}>
        <div className="footer-inner" style={{ maxWidth: 1160, margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap' as const, gap: 32 }}>
          <div>
            ...
            <p style={{ fontSize: 13, lineHeight: 1.65, maxWidth: 220 }}>India's zero-fee UPI payment gateway. Accept payments online — direct to your bank account.</p>
          </div>
